...
/// `init_fs`-style call, for a future `process_init` macro to call
...
/// A spawned child process. `wait`/`wait_with_output` consume `self`
/// (ownership transfers into the pool closure, so nothing needs to be
/// shared or locked); `kill`/`id` are synchronous since they're fast,
...
impl DtactChild {
    fn new(inner: std::process::Child) -> Self {
        Self { inner }
    }
...
macro_rules! child_pipe {
    ($name:ident, $inner:ty) => {
        /// One end of a child process's stdio pipe. Exclusively owned by
        /// whoever holds it (returned by `take_std*` on `DtactChild`) —
        /// each async op temporarily moves the handle into a pool
        /// closure and gets it back in the result, never shared behind a
        /// lock.
