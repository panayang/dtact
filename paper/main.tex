...
DTA and \tokio{}~\cite{tokio} differ in at least three architectural dimensions
beyond the scheduling algorithm:
\begin{enumerate}
  \item \textbf{Async model.}
        DTA uses \emph{stackful} fibers (each task carries a full
        saved register context); \tokio{} uses \emph{stackless}
        futures (state-machine compilation).  A \texttt{yield\_now()}
        in DTA therefore incurs a full context-switch, while in \tokio{}
        it is a state-machine transition with no stack save.
  \item \textbf{Memory allocation strategy.}
        DTA pre-allocates a fixed context pool at startup; allocation
        cost per task is $O(1)$ pointer arithmetic.
  ...
  \item \textbf{Work-routing protocol.}
        DTA uses push-based deflection (SPSC mailboxes);
        \tokio{} uses pull-based work-stealing (Chase--Lev deque~\cite{chase_lev2005}).
\end{enumerate}
Benchmarks that favour DTA (high spawn rate, batch workloads) may
partly reflect pool-allocation and SPSC-write advantages rather than
scheduling algorithm superiority.  Benchmarks where \tokio{} wins
(cooperative yield) directly reflect the stackful-vs-stackless cost
asymmetry.  The theoretical model accounts only for the scheduling
algorithm; the architectural differences are unmodelled confounders
...
This regression is not a bug but a \textbf{design boundary}
(see Remark~\ref{rem:confounders} for the full list of confounders):
DTA is architected for \emph{BoT spawn-and-complete} workloads, not for
