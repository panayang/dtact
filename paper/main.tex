...
\textbf{Version History} \\
      \small v1: June 28, 2026
\end{center}

\vspace{1.2em}

\begin{abstract}
This paper presents a unified formal mathematical analysis of the DTA
distributed task scheduler.  Using a second-quantisation-inspired operator
formalism, we establish: (I) correctness properties --- no task loss, deadlock
freedom, livelock freedom, and linearizability~\cite{herlihy_wing1990}; (II) progress properties ---
bounded waiting time and starvation freedom; (III) quantitative performance
bounds --- load balance, kinetic-theory corrections to mean-field, makespan
bounds via extreme-value theory, and a four-level stability ladder; and
(IV) competitive and empirical analysis --- online competitive ratio~\cite{borodin_elyaniv1998,sleator_tarjan1985} relative
to work stealing, adversarial stability comparison, and correspondence with
Criterion benchmark measurements on GitHub Actions CI.  All theoretical
predictions are numerically validated and, where applicable, matched against
empirical wall-clock data.
\end{abstract}
...
result for SPSC queues is folklore (see, e.g., \cite{boehm2008}
...
\midrule
\code{tail} CAS (success)   & \code{Relaxed}/\code{Relaxed} & $\geq$ Relaxed \checkmark \\
\code{seq.load} (probe)     & \code{Acquire}                & Acquire \checkmark \\
\code{seq.store} (publish)  & \code{Release}                & Release \checkmark \\
\code{head} CAS (success)   & \code{Relaxed}/\code{Relaxed} & $\geq$ Relaxed \checkmark \\
\code{seq.store} (reclaim)  & \code{Release}                & Release \checkmark \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{remark}[Validity of Assumption~\ref{ass:exch}]
The actual routing uses a deterministic hash $(h_1 + h_2) \bmod N$
derived from \code{flow\_id} bits, not a uniform random target.
Under a uniform distribution of \code{flow\_id}, the hash is
equidistributed mod $N$, making the uniform-random-target
approximation exact in distribution for i.i.d.\ flow IDs.
The CCX-topology modes break this symmetry slightly (same-CCX
peers are preferred); the mean-field result below gives the
\emph{topology-unaware} baseline against which CCX-aware behavior
can be compared.
\end{remark}
...
\emph{zeroth-order term} of a systematic BBGKY expansion in powers of $1/N$,
...
(Bogoliubov--Born--Green--Kirkwood--Yvon).  Closure requires a truncation
...
We rewrite~\eqref{eq:bbgky1} in the standard kinetic-theory form:
\begin{equation}
...
In classical kinetic theory of identical particles, the scattering kernel
...
(propagation of chaos~\cite{mckean1967,sznitman1991}~\cite{mckean1967,sznitman1991}; cf.\ Kac 1956, McKean 1966).
...
\midrule
Derivation & Master equation, exchangeability & BBGKY hierarchy \\
...
% =============================================================
\section{Line 2: Statistical Bound via Extreme Value Theory~\cite{leadbetter1983,fisher_tippett1928}~\cite{leadbetter1983,gumbel1958}}
\label{sec:stat}
% =============================================================
...
\midrule
DTA competitive ratio (BoT) & $1 + p_d\delta_{\mathrm{hop}}\mu/(1-p_d) + O(\sqrt{\ln N}/\bar q)$ & Theorem~\ref{thm:cr_bot} \\
DTA large-$H$ limit & $\to 1$ as $H \to \infty$ & Corollary~\ref{cor:large_H} \\
Benchmark system & $\varrho_c \lesssim 1.12$ & Corollary~\ref{cor:benchmark} \\
Heavy-tail penalty & $+\,(1+\mathrm{CV}^2)/N$ & Proposition~\ref{prop:heavy_tail} \\
WS competitive ratio (BoT) & $1 + (1-\varrho^*)\delta_{\mathrm{steal}}\mu$ & Theorem~\ref{thm:cr_ws_bot} \\
WS for DAGs & $T_1/N + O(T_\infty)$ & \cite{blumofe1999} \\
\bottomrule
\end{tabular}
\end{center}
...
\legend{Run 1, Run 2 (independent, later commit)}
\end{axis}
\end{tikzpicture}
