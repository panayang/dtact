...
\textbf{Version History} \\
      ...
      \small v7: July 2, 2026 \quad v8: July 2, 2026 \\
      \small v9: July 2, 2026
\end{center}
...
free design choice or a real cost, and if it is a cost, how large. Answering
it requires three separate arguments, each building on the last. First, a
scheduler that observes less must still be shown \emph{safe}: with a
smaller information set there is more room for a task to be lost, for two
workers to deadlock waiting on each other's mailboxes, or for a chunk to
loop forever between peers instead of being scheduled (Parts~I--II).
Safety alone is not enough, however, since a scheduler can be safe and
still make every worker wait arbitrarily long for work; we therefore prove
an explicit, distribution-free bound on waiting time before any of the
performance analysis can be trusted (Part~III). Only once correctness and
progress are established does it make sense to ask how \emph{good} the
restricted-information design is quantitatively, and we answer this both
under a stochastic workload model (Part~IV, mean-field and kinetic theory)
and under a worst-case adversarial one (Part~V, competitive analysis and
empirical validation) --- two regimes that make no assumptions in common,
so agreement between them is itself evidence that the bound is not an
artefact of either model. Finally, we close the loop: Parts~I--V establish
that DTA \emph{works} despite restricted information, but not yet what
that restriction is worth, so Part~VI gives an explicit accounting of how
much information a comparable work-stealing scheduler must acquire to
match it, together with the hardware cost of acquiring that information.
Figure~\ref{fig:logic-chain} summarises how these parts connect to the
central question, and the ``Organization'' paragraph below gives the
section-level map. The overall picture that emerges is that bounded-information
scheduling is not a compromise made for the sake of simplicity; on the
workloads and hardware regimes we characterise, it is close to
information-optimal, and it is strictly cheaper to run.
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, minimum height=9mm,
              minimum width=26mm, fill=blue!6, inner sep=3pt},
  qbox/.style={draw, rounded corners, align=center, minimum height=9mm,
               minimum width=26mm, fill=orange!10, inner sep=3pt},
  cbox/.style={draw, rounded corners, align=center, minimum height=11mm,
               minimum width=52mm, fill=green!8, inner sep=3pt},
  arr/.style={-{Latex[length=2mm]}, thick}
]
  \node[cbox] (q) {Central question: is bounded-information routing a free\\
                    design choice, or does it cost something -- and how much?};
  \node[box, below left=9mm and 6mm of q] (safety) {Part I--II\\Safety: no lost tasks,\\no deadlock/livelock};
  \node[box, below=of safety] (progress) {Part III\\Progress: explicit\\waiting-time bound $W_{\max}$};
  \node[box, below right=9mm and 6mm of q] (stoch) {Part IV\\Stochastic quality:\\mean-field / kinetic theory};
  \node[box, below=of stoch] (advers) {Part V\\Worst-case quality:\\competitive ratio + benchmarks};
  \node[qbox, below=16mm of q] (cost) {Part VI\\Price of the restriction:\\information-cost framework ($\eta_{\mathrm{info}},\eta_{\mathrm{util}}$)};

  \draw[arr] (q.west) .. controls +(-8mm,-4mm) .. (safety.north);
  \draw[arr] (safety) -- (progress);
  \draw[arr] (q.east) .. controls +(8mm,-4mm) .. (stoch.north);
  \draw[arr] (stoch) -- (advers);
  \draw[arr] (progress.south) .. controls +(0,-10mm) and +(-16mm,6mm) .. (cost.west);
  \draw[arr] (advers.south) .. controls +(0,-10mm) and +(16mm,6mm) .. (cost.east);
  \node[align=center, below=2mm of cost, font=\footnotesize\itshape, text width=90mm]
    {Answer: correctness and progress hold unconditionally (I--III);
     quality is near-optimal in both regimes (IV--V);
     the restriction is quantifiably cheap, not merely tolerable (VI).};
\end{tikzpicture}%
}
\caption{Logic chain of the paper. Safety (Parts~I--II) and progress
(Part~III) must hold before any performance claim is meaningful; quality is
then established independently under a stochastic workload model
(Part~IV) and an adversarial one (Part~V); Part~VI closes the loop by
pricing the information restriction whose safety and quality were just
established. Each arrow is a logical dependency, not merely a reading
order.}
...
\begin{itemize}
  ...
  \item The CAS target (\code{Warehouse::tail} or \code{Warehouse::head})
        ...
        The system-wide lock-free guarantee alone only bounds
        \emph{aggregate} progress (some contender's CAS succeeds on
        every collision); it does not by itself bound any
        \emph{individual} worker's retry count, which could in
        principle be starved by an adversarial scheduler that always
        lets a different contender win. We add one standard fairness
        assumption to close this gap: a \emph{fair scheduler} does not
        systematically bias the winner of a CAS race away from any
        fixed worker, i.e., conditional on a collision occurring among
        the (at most $N-1$ other) contenders active at that instant,
        each contending worker's CAS wins with probability at least
        $1/N$. Under this assumption, worker $i$'s retry count $R_i$ is
        stochastically dominated by a $\mathrm{Geometric}(1/N)$ random
        variable (each attempt independently succeeds with probability
        $\geq 1/N$, conditionally on a retry being needed at all), giving
        the explicit bound $\mathbb{E}[R_i] \leq N$ --- not merely
        $\mathbb{E}[R_i]<\infty$, but a concrete worst-case constant in
        terms of the contention width. The exponential back-off
        (Assumption~\ref{ass:live}-compatible: it only ever delays,
        never blocks, this worker) additionally spreads out repeated
        self-collisions in time without affecting this bound's validity,
        since the domination argument only uses the per-attempt success
        probability, not the inter-attempt delay.
        A finite-a.s.\ (indeed finite-mean) retry count is exactly what
        ruling out deadlock (Definition~\ref{def:deadlock}) requires: it
        precludes \emph{permanent} blocking.
\end{itemize}
...
a worst-case bound that requires no distributional assumption on
$\lambda$ or $\mu$, holding for every arrival pattern subject to one
pathwise rate condition on the warehouse (Assumption~\ref{ass:wh-net-drain}:
the warehouse's net inflow during any busy interval must not exceed its
collective drain rate, needed so that a worker's mailbox polling cannot
be delayed indefinitely by sustained peer overflow). Both bounds are
derived from the local dispatch-loop structure and the deterministic
polling order, without any assumption that a worker knows the state of
its peers --- a direct consequence of the bounded-information design.
...
\begin{assumption}[Warehouse net-drain condition]
\label{ass:wh-net-drain}
During any maximal interval in which the warehouse is continuously
observed busy (i.e., \code{warehouse.is\_busy()} holds at the start of
every worker iteration throughout the interval), the aggregate rate at
which \emph{other} workers push new chunks into the warehouse does not
exceed the collective drain rate $N/(K\delta)$ chunks per unit time.
This is a pathwise, worst-case rate cap on the arrival pattern (not a
distributional/steady-state average): it holds automatically whenever
the mean-field warehouse-stability condition
$\Lambda_W < \Lambda_D$ of Theorem~\ref{thm:wh-stable}
(\hyperref[chap:stability]{Performance~IV}) holds, but is stated here
directly as a worst-case condition so that this chapter's bound remains
distribution-free. Without some such condition, sustained concurrent
overflow from other workers could in principle keep a worker's
observed backlog nonzero indefinitely, and since
Definition~\ref{def:iteration} gates \code{poll\_mailboxes} on
\code{warehouse.is\_busy()}$=$false, that worker would never resume
polling --- so the bound below is conditional on
Assumption~\ref{ass:wh-net-drain}, not unconditional over every
conceivable arrival pattern.
\end{assumption}

\begin{lemma}[Warehouse drain time (collective)]
\label{lem:wdrain}
Under Assumption~\ref{ass:wh-net-drain}, if the warehouse contains at
most $C_W \cdot K$ tasks at the start of a busy interval, all $N$
workers collectively empty it within that interval in at most
\[
...
\begin{proof}
The warehouse holds at most $C_W$ chunks at the start of the interval,
and by Assumption~\ref{ass:wh-net-drain} the net inflow during the
interval is non-positive relative to the collective drain rate. Each
worker drains $D$ chunks per iteration of the drain phase, spending
$DK\delta$ dispatching them.  The total chunk-drain capacity of all $N$
workers per unit time is $N \cdot D / (DK\delta) = N/(K\delta)$
chunks per time unit, so the time to drain (at most) $C_W$ net chunks
is $C_W K\delta / N$.  (The factor $K$ accounts for the dispatch
...
for the duration of the drain phase.  Under
Assumption~\ref{ass:wh-net-drain}, Lemma~\ref{lem:wdrain} bounds this
drain phase at $T_{\mathrm{drain}} = C_W K\delta/N$, after which
\code{warehouse.is\_busy()} reports false and polling resumes on the
next iteration, so $\tau$ is picked up within the next poll cycle
($\leq L\delta$).
...
\[
  W_W \;\leq\; \ceil{\frac{C_W K}{NDK}} \cdot DK\delta
       ...
       \;\leq\; \left(\frac{C_W}{N} + D\right)\delta K
\]
using $\lceil x \rceil \le x+1$ termwise. When $ND$ divides $C_W$
exactly (as in every configuration considered in this paper, e.g.\ the
benchmark parameters of \S\ref{sec:validity}), the ceiling is exact and
this reduces to $W_W \leq \frac{C_WK}{N}\delta$, the value used in the
main theorem below; in general the extra additive slack is at most
$D\delta K$, which is dominated by the $L\delta$ term already present
in the theorem's bound whenever $D \leq L$ (true for all published
configurations, since $D \le C_W/N \ll L$).
...
Under Assumptions~\ref{ass:smax}--\ref{ass:live} and~\ref{ass:wh-net-drain},
for any task $\tau$ admitted to the DTA scheduler:
\[
  \boxed{
...
\begin{proposition}[Positive correlation of simultaneous overload
(overload contagion)]
\label{prop:anticorr}
In steady state, $\gtwo(q, q') > 0$ for $q \geq H$ and $q' \geq H$: the
event that two peers are simultaneously overloaded is \emph{more}
likely than mean-field independence predicts.
\end{proposition}

\begin{remark}[Why naive reasoning suggests the wrong sign]
It is tempting to reason that once both workers are already overloaded,
a deflection between them cannot succeed (the receiver condition
$q_j < H$ fails), and to read this as evidence of anti-correlation. But
that reasoning addresses the wrong event --- it describes what happens
\emph{after} the pair has already jointly reached $(H,H)$, not the
\emph{rate of arriving there}. The relevant comparison is the flux
\emph{into} $(H,H)$, and the deflection channel (item~2 of
\S\ref{sec:mmp-rates}, rate $\lambda_0/(N-1) \cdot
\mathbf{1}[q_j \geq H]\mathbf{1}[q_i<H]$) only \emph{exists} because a
peer is overloaded: an overloaded worker $j$ actively redirects its own
excess arrivals onto its peers, strictly raising each peer's inbound
rate above the mean-field value $\lambda_0$ whenever $j$ is full. This
is a positive feedback (``contagion''), not a negative one, and it
dominates the receiver-side saturation effect described above.
\end{remark}

\begin{proof}
We give two arguments: a general rate-comparison argument, and an
exact closed-form verification for the smallest non-trivial case
($N=2$, $H=1$), which the reader can check independently.

\medskip
\noindent\textbf{General argument.} Fix two peers $i,j$ and consider
the flux into the state $q_i = H$ conditional on $q_j$. By the
transition rates of \S\ref{sec:mmp-rates}, the total upward rate into
$q_i$ is $\lambda_0\mathbf{1}[q_i<H]\bigl(1 + \frac{1}{N-1}\sum_{k\neq i}
\mathbf{1}[q_k \geq H]\bigr)$, which is \emph{strictly increasing} in
the number of overloaded peers --- in particular, strictly larger when
$q_j \geq H$ than when $q_j < H$. Since $i$'s upward rate (and hence its
probability of reaching $H$) is monotonically larger conditional on
$q_j \geq H$, and this holds symmetrically for $j$ conditional on
$q_i \geq H$, the events $\{q_i \geq H\}$ and $\{q_j \geq H\}$ are
positively associated: $\bP(q_i \geq H \mid q_j \geq H) >
\bP(q_i \geq H \mid q_j < H)$, which is equivalent to
$\gtwo(q,q') > 0$ for $q,q' \geq H$ (this is a standard consequence of
the FKG-type monotonicity of birth rates in the peer's state; see
e.g.\ the association arguments of~\cite{liggett1985} for
interacting-particle systems with monotone rates).

\medskip
\noindent\textbf{Exact verification, $N=2$, $H=1$.} With two workers
and threshold $1$, each queue is capped at $q_i \in \{0,1\}$ (once
$q_i = H$, both upward channels of \S\ref{sec:mmp-rates} are gated off
by $\mathbf{1}[q_i<H]$, so the state space is exactly
$\{0,1\}^2$). Writing $\rho \coloneqq \lambda_0/\mu$ and solving the
$4$-state CTMC balance equations exactly (by symmetry $\pi_{01}=\pi_{10}$):
\[
  \pi_{00} = \frac{1}{D}, \qquad
  \pi_{01} = \pi_{10} = \frac{\rho}{D}, \qquad
  \pi_{11} = \frac{2\rho^2}{D}, \qquad
  D \coloneqq 1 + 2\rho + 2\rho^2.
\]
Then $\bP(q_1{=}1) = \bP(q_2{=}1) = \rho(1+2\rho)/D$, and
\[
  \gtwo(1,1) \;=\; \pi_{11} - \bP(q_1{=}1)\bP(q_2{=}1)
  \;=\; \frac{2\rho^2}{D} - \frac{\rho^2(1+2\rho)^2}{D^2}
  \;=\; \frac{\rho^2}{D^2}
  \;>\; 0
\]
for every $\rho > 0$, confirming the general argument in the exactly
solvable case. (We verified this identity, and its extension to
$N=3,4,5$ by direct numerical solution of the corresponding CTMC, which
shows $\gtwo(H,H) > 0$ throughout and $(N-1)\gtwo(H,H)$ converging to a
finite limit as $N$ grows, consistent with the $O(1/(N{-}1))$ magnitude
of Proposition~\ref{prop:1overN}.)
\end{proof}

\begin{corollary}[Sign of the correction]
The correction $\Delta\pd^{(q)} = \fone(q)^{-1} \sum_{q' \geq H} \gtwo(q, q')$
is \emph{positive} for all $q$: the true deflection probability is
\emph{higher} than the mean-field prediction.
Equivalently, the mean-field approximation \emph{underestimates} the
coupling between workers, and the true effective traffic intensity
satisfies $\rho^{(2)} > \rhostar_{\mathrm{MF}}$. Mean-field is, in this
sense, an \emph{optimistic} (not conservative) approximation of the
true deflection load; see the discussion of downstream consequences in
\S\ref{sec:contagion-scope}.
\end{corollary}
...
Retaining only the level-1 term and using the (corrected) positive
contagion sign of Proposition~\ref{prop:anticorr}:
\begin{equation}
  \pd^{(2)}
  \;\approx\; \pd\Bigl(1 + \frac{\rho_0\,\pd}{N-1}\Bigr),
  \label{eq:pd-level1}
\end{equation}
where the factor $\rho_0\pd/(N-1)$ measures the fractional \emph{increase}
in deflection probability due to pairwise overload contagion (mean-field
underestimates $\pd$). For the benchmark ($N=4$, $\rho_0 \leq 0.9$,
$\pd \approx 0$ at moderate load), this correction is of order
$\rho_0\pd/3 \approx 0$, i.e.\ negligible at moderate load regardless of
sign --- consistent with the magnitude (not sign, which was not
separately resolved by that measurement) of the numerical observation
$\epsilon_N \approx 0.0003$--$0.0004$ in
\texttt{simulate\_load\_balance.py}.
At high load ($\rho_0 \to 1$, $\pd \to 1$) and small $N$,
the correction approaches $1/(N-1)$; for $N=4$ this is $\approx 33\%$,
now understood as an \emph{under}-estimate of the true deflection
probability by mean-field at that regime, not an over-estimate. See
\S\ref{sec:contagion-scope} for what this implies for bounds derived
elsewhere in this paper from the mean-field value of $\Delta\ell$.
\end{remark}
...
\begin{remark}[Sign of the $O(1/N)$ term]
This theorem's inequality is written with a $(1+O(1/N))$ \emph{growth}
factor, consistent with $\gtwo(H,H)>0$ (Proposition~\ref{prop:anticorr}):
the $O(1/N)$ term here is \emph{positive}, so
$\Delta\ell^{(2)} \geq \Delta\ell_{\mathrm{MF}}$ (level-1 imbalance is
genuinely larger than mean-field predicts, not merely ``at most'' as
large). The proof below only uses the \emph{magnitude} bound
${\rhostar}^{(2)} = \rhostar + O(1/N)$ from Proposition~\ref{prop:1overN},
...
% =============================================================
\subsection{Scope: what this means for bounds derived from $\Delta\ell_{\mathrm{MF}} \leq 1/2$ elsewhere in this paper}
\label{sec:contagion-scope}
% =============================================================

Several results outside this chapter cite ``$\Delta\ell \leq 1/2$''
(Theorem~4.1 of \hyperref[chap:load_balance]{Performance~I}) as a bound
on the true, finite-$N$ system, when strictly it bounds only the
\emph{mean-field} imbalance $\Delta\ell_{\mathrm{MF}}$; the true,
level-1-corrected imbalance can exceed it by a positive $O(1/N)$ term
(overload contagion). We checked every downstream use:
\hyperref[chap:makespan]{Performance~III}'s statistical and composite
makespan bounds (Theorems~\ref{thm:stat-makespan},~\ref{thm:composite})
use $\Delta\ell \leq 1/2$ only inside an already-budgeted
$O(\cdot)$-level coefficient, so the correction changes their stated
constant by a further $(1+O(1/N))$ factor but not their validity; the
competitive-ratio chapter's Theorem~\ref{thm:cr_bot} uses $\bar q$ and
$\mathbb{E}[M_N]$ directly rather than $\Delta\ell \leq 1/2$, so it is
formally unaffected, though replacing $\rhostar$ with the
contagion-corrected ${\rhostar}^{(2)}>\rhostar$ would shift
Corollaries~\ref{cor:large_H}--\ref{cor:benchmark}'s numeric
illustrations slightly higher (worse for DTA) at small $N$; and the
distribution-free worst-case bounds
(Theorem~\ref{thm:cr_bot_worst}, Progress~I's bounded-wait theorem) do
not use $\Delta\ell$ at all and are entirely unaffected.

\noindent\textbf{Bottom line.} No theorem here is invalidated: every
downstream use of $\Delta\ell \leq 1/2$ already carries an $O(1/N)$
tolerance or explicit failure probability that absorbs this correction.
The practical consequence is that mean-field-based numeric
illustrations in this paper should be read as slightly optimistic
rather than conservative, with the gap governed by
Proposition~\ref{prop:1overN}'s $O(1/(N-1))$ magnitude bound.
...
Overload contagion & Not modeled & Captured by $\gtwo > 0$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
  ...
  \item The connected pair-correlator $\gtwo$ is $O(1/N)$
        (Proposition~\ref{prop:1overN}) and \emph{positive} for
        $q, q' \geq H$ (Proposition~\ref{prop:anticorr}: overload
        contagion, not anti-correlation).
        This means the mean-field \emph{underestimates} deflection
        intensity; the correction is $\Delta\pd \approx +\rho_0\pd^2/(N-1)$.
...
\begin{equation}
  ...
  \label{eq:lyapunov}
\end{equation}
The generator drift satisfies, for all $\mathbf{q}$:
\begin{equation}
  \generator V(\mathbf{q})
  \;\leq\; -2\mu\norm{\mathbf{q}}_1 + N\bigl(\lambda_0(2H+1)+\mu\bigr).
  \label{eq:drift}
\end{equation}
Consequently, DTA is throughput-stable (positive recurrent) for
\emph{every} arrival rate $\lambdazero$, including $\rhozero \geq 1$ ---
a structural consequence of the admission cap at $H$, not a threshold
phenomenon at $\rhozero=1$. The $\rhozero<1$ threshold instead governs
the \emph{scale} of the stationary queue length (Corollaries
\ref{cor:mean-queue}--\ref{cor:instability}), which is the practically
relevant distinction.
\end{theorem}

\begin{proof}
The generator acts on $V$ coordinate-wise. Let $m = m(\mathbf{q})
\coloneqq |\{j : q_j \geq H\}|$ be the (state-dependent) number of
currently-full workers. For a non-full worker $i$ ($q_i<H$), the exact
transition rates of \S\ref{sec:mmp-rates} give effective arrival rate
$\lambda_{\mathrm{eff},i} = \lambda_0\bigl(1 + \tfrac{m}{N-1}\bigr)$
--- identical across all $N-m$ non-full workers, since the deflection
boost each receives depends only on the number of \emph{other} workers
that are full. (This is an exact pathwise identity, not the mean-field
average $\mu\rhostar$: an earlier version of this proof substituted
$\lambda_{\mathrm{eff},i}\leq\mu\rhostar$ here, which only bounds the
rate \emph{on average} over the mean-field stationary distribution of
peers, not at every reachable state --- the true worst case is
$\lambda_{\mathrm{eff},i} \to 2\lambda_0$ as $m\to N-1$.) For $q_i \geq
H$, arrivals are deflected outward, so $\lambda_{\mathrm{eff},i} = 0$.
Using $(q+1)^2-q^2=2q+1$ and $(q-1)^2-q^2=-2q+1$, and writing
$c(m) \coloneqq \lambda_0(1+m/(N-1))$:
\[
  \generator V(\mathbf{q})
  = \underbrace{c(m)\!\!\sum_{i:\,q_i<H}\!\!(2q_i+1)}_{\text{arrivals, non-full workers}}
  \;-\; 2\mu\!\!\sum_{i:\,q_i>0}\!\!q_i \;+\; N\mu.
\]
Since each non-full $q_i < H$, the arrivals term is at most
$c(m)(N-m)(2H+1)$. The function $c(m)(N-m)$ is maximised over
$m\in\{0,\ldots,N-1\}$ at $m=0$ (equivalently $m=1$, by direct
computation), where it equals $N\lambda_0$ exactly: this is a flow-conservation
fact (deflection only \emph{redistributes} the $N\lambda_0$ raw external
arrival rate among workers; it cannot manufacture additional arrivals),
so the arrivals term is bounded by the \emph{constant}
$N\lambda_0(2H+1)$, independent of $\mathbf{q}$. Hence
\[
  \generator V(\mathbf{q})
  \;\leq\; N\lambda_0(2H+1) + N\mu \;-\; 2\mu\norm{\mathbf{q}}_1.
  \label{eq:perworker-drift}
\]
This is a valid Foster--Lyapunov drift condition with $\epsilon=2\mu$
and $b = N(\lambda_0(2H+1)+\mu)$, valid \emph{pathwise at every state},
with no substitution of a mean-field average for a per-state quantity.
By the Foster--Lyapunov theorem \cite{meyn2009}, the chain is positive
recurrent. Unlike the flawed substitution this replaces, this argument
does not actually require $\rhozero<1$: because $q_i\le H$ is already
enforced structurally (arrivals are deflected once a worker reaches
$H$), the chain lives on the finite state space $\{0,\ldots,L\}^N$ and
is positive recurrent for \emph{any} $\rhozero$, including
$\rhozero\geq 1$. The content of the $\rhozero<1$ vs.\ $\rhozero\geq 1$
distinction is therefore not recurrence itself but the \emph{scale} of
the stationary queue length, addressed by the corollaries below.
\end{proof}

\begin{corollary}[Mean queue length bound]
\label{cor:mean-queue}
In steady state, $\mathbb{E}[\norm{\mathbf{q}}_1] \leq b/\epsilon =
N(\lambda_0(2H+1)+\mu)/(2\mu)$, a pathwise-valid but $H$-loose bound
that holds for every $\rhozero$ (see the proof above). The
qualitatively sharper, $\rhozero$-dependent estimate
$\mathbb{E}[q_i^*] \approx \rhostar/(1-\rhostar)$, which diverges as
$\rhostar \to 1$ and saturates to $H$ consistent with the M/M/1/$H$
mean queue, is the mean-field prediction of
\hyperref[chap:load_balance]{Performance~I}'s self-consistency
equation, independently cross-validated against CTMC simulation
throughout this paper (e.g.\ \S\ref{sec:sim},
\hyperref[chap:makespan]{Performance~III}); it is not re-derived
pathwise here.
\end{corollary}
...
The admission cap at $H$ is what makes Theorem~\ref{thm:lyapunov}'s
positive recurrence unconditional: once $q_i \geq H$, worker $i$'s
arrival rate drops to zero and only the negative drift $-2\mu q_i$
remains, so no worker's local queue can ever be a source of unbounded
growth by itself. This structural cap is also why the interesting
question is not \emph{whether} the chain is recurrent but \emph{where}
its stationary mass concentrates: for $\rhozero<1$ the mean-field
self-consistency equation predicts $\mathbb{E}[q_i^*]$ small relative
to $H$ (Corollary~\ref{cor:mean-queue}'s sharper estimate), while for
$\rhozero\geq 1$ it predicts saturation at $q_i\approx H$
(Corollary~\ref{cor:instability}), pushing excess load onto the
warehouse.
\end{remark}
...
For the full $N$-worker system under mean-field coupling, the generator
is approximately a sum of $N$ independent single-worker generators with
the same effective rate $\lambda_{\mathrm{eff}}$. Since the generator of
a product chain is a Kronecker \emph{sum}, its eigenvalues are sums of
the single-chain eigenvalues; the spectral gap of the product (the
smallest nonzero eigenvalue) is therefore $\gamma$, not $N\gamma$ ---
achieved by perturbing a single coordinate while the other $N-1$ sit at
their own stationary (zero-eigenvalue) component. Consequently the
mixing time scales as $t_{\mathrm{mix}}^{(N)} \sim \gamma^{-1}$,
independent of $N$: there are $N$ ``channels'' relaxing simultaneously
and independently, but the slowest of them still sets the pace, so the
product's gap does not improve on any single channel's.
\end{remark}
...
\begin{itemize}
  \item \textbf{Input rate} $\Lambda_W$: the rate at which tasks arrive
        at the warehouse when all deflection paths are saturated. The
        deflection protocol tries at most $h_{\max} = \lfloor N/2\rfloor$
        peers (\S\ref{sec:dag_kinetic}, Definition of the hop bound) before
        parking a chunk in the warehouse --- \emph{not} all $N-1$ peers.
        In mean-field, treating the $h_{\max}$ sampled peers as an
        independent sample (Stosszahlansatz), a worker deflects to the
        warehouse when all $h_{\max}$ tried peers have $q_j \geq H$
        simultaneously:
        \begin{equation}
          \Lambda_W \;=\; N\lambdazero\, \pd^{\lfloor N/2\rfloor},
          \label{eq:wh-input}
        \end{equation}
        where $\pd = \pi(H;\rhostar)$ is the per-worker probability of
        being above threshold; using $\lfloor N/2\rfloor \le N-1$ gives a
        strictly larger (more pessimistic, and more accurate) $\Lambda_W$
        than an all-$N-1$-peers model for $\pd<1,N>3$.
...
\begin{equation}
  \Lambda_W < \Lambda_D
  \quad\Longleftrightarrow\quad
  \rhozero\,\pd^{\lfloor N/2\rfloor} < 1 - \pd.
  \label{eq:wh-stable}
\end{equation}
\end{theorem}
...
\begin{equation}
  \rhozero^{\dagger}
  \;\coloneqq\;
  \frac{1-\pd^*(\rhozero^{\dagger})}{(\pd^*(\rhozero^{\dagger}))^{\lfloor N/2\rfloor}},
  \label{eq:rho-dagger}
\end{equation}
the warehouse stability condition~\eqref{eq:wh-stable} is satisfied.
For moderate load ($\pd \ll 1$), $\rhozero^{\dagger} \approx 1/\pd^{\lfloor N/2\rfloor} \gg 1$,
...
\begin{equation}
  ...
  \;=\; \frac{C_W}{N(\rhozero\pd^{\lfloor N/2\rfloor}\mu - \mu(1-\pd))}.
  \label{eq:crash-time}
\end{equation}
...
At $\rhozero = 0.95$ with $N=4$ ($\lfloor N/2\rfloor = 2$) and
$\pd = \pi(H;\rhostar)$ evaluated at the self-consistent $\rhostar$:
\[
  T_{\mathrm{crash}} \approx \frac{C_W}{N\mu(\rhozero\pd^2 - (1-\pd))}.
\]
For $\pd \ll 1$ (even at $\rhozero = 0.95$, $\pd$ is small),
the denominator $\approx N\mu\pd^2 \approx 0$, giving
...
system is near saturation. (Using $\lfloor N/2\rfloor=2$ rather than $N-1=3$
makes this threshold easier to cross than a naive all-peers model would
suggest, though still requiring $\pd$ not small.)
\end{corollary}
...
$\Lambda_W > \Lambda_D$, which requires $\pd^{\lfloor N/2\rfloor} > (1-\pd)/\rhozero$.
At benchmark parameters with $\rhozero \leq 0.95$, $\pd^2 \ll (1-\pd)$
...
$\bar q$. The same calculation at higher load ($\varrho_0=0.95$,
$\bar q \approx 19$) gives imbalance factor $\approx 3.2$ --- still not
$\lesssim 1.12$, though substantially improved, illustrating the slow
(logarithmic in $N$, linear in $\bar q$) approach to the heavy-traffic
limit. At
the $N=256$ target deployment scale and moderate-to-high load, the
informative bound on DTA's competitive ratio is therefore not this
...
\begin{remark}[Scope: this theorem compares overhead terms, not the full ratio]
\label{rem:regime_compare_scope}
Theorem~\ref{thm:regime_compare} compares only the \emph{routing/stealing-overhead}
components of $\varrho_c^{\mathrm{DTA}}$ and $\varrho_c^{\mathrm{WS}}$
($p_d\delta_{\mathrm{hop}}$ vs.\ $(1-\varrho^*)\delta_{\mathrm{steal}}$); it does
not claim the \emph{full} competitive ratio of Theorem~\ref{thm:cr_bot}
(routing $\times$ imbalance) converges to $1$ at fixed $H$ --- that requires
$H\to\infty$ too (Corollary~\ref{cor:large_H}), and the imbalance factor alone
can remain large ($\gtrsim3$--$6$, Corollary~\ref{cor:benchmark}). Accordingly,
Figure~\ref{fig:cr_vs_rho}'s full empirical ratio tracks WS/CQ rather than
beating them outright: ``DTA dominates at all loads'' in item~1 above is an
overhead-only statement.
\end{remark}
...
\begin{equation}\label{eq:cas_per_task}
  ...
  \;=\;
  \frac{\mu\,(1-\varrho_0)\,N}{\lambda_0}
  \;=\;
  \frac{N(1-\varrho_0)}{\varrho_0},
\end{equation}
...
\begin{equation}\label{eq:ws_low_load_overhead}
  ...
  \;\approx\;
  \frac{N(1-\varrho_0)}{\varrho_0} \cdot (\delta_{\mathrm{CAS}} + \delta_{\mathrm{coh}}),
\end{equation}
where $\delta_{\mathrm{coh}} \approx (N-1)\cdot\delta_{\mathrm{MESI}}$ is
the cache-coherence round-trip cost of a single failed CAS (MESI protocol
invalidation broadcast to the other $N-1$ caches); the $(N-1)$ factor
enters only here, per-failed-CAS, not as a multiplier on the number of
failed CAS operations itself.
\end{proposition}
...
Total steal attempts per unit time across $N$ idle workers $\approx N(1-\varrho_0)/\delta_{\mathrm{CAS}}$.
...
\emph{succeed} in rebalancing load; it ignores the $O(N/\varrho_0)$
...
\caption{Empirical competitive ratio $\varrho_c$ (batch LPT-vs-\{DTA, Work-Stealing,
  Central Queue, Random\}, $N=8$, $\mathrm{Exp}(\mu)$ and $\mathrm{Pareto}(2.5)$ task
  sizes, 60 Monte Carlo batches per point; the deflection watermark $H$ and batch
  size $M$ scale with $\varrho_0$ so that deflection is non-trivial), versus the
  routing/stealing overhead-only terms of Theorem~\ref{thm:cr_bot} and
  Theorem~\ref{thm:cr_ws_bot} (dotted). DTA tracks Work-Stealing and Central Queue
  closely and stays well below Random; see the discussion below and
  Remark~\ref{rem:regime_compare_scope} for why the solid curves sit above the
  overhead-only dotted lines and for the scope of the ``DTA dominates'' claim.}
\label{fig:cr_vs_rho}
\end{figure}
...
\]
Figure~\ref{fig:cr_vs_rho} (see \texttt{simulate\_competitive.py}) plots the
empirical competitive ratio $\varrho_c = 1/\eta$ as a function of $\varrho_0$
for DTA, Work-Stealing, Central Queue, and Random on BoT tasks, together with
the overhead-only terms of Theorems~\ref{thm:cr_bot} and~\ref{thm:cr_ws_bot}.

The key qualitative features from Theorems \ref{thm:cr_bot}--\ref{thm:regime_compare}
concern the \emph{overhead-only} terms specifically (Remark~\ref{rem:regime_compare_scope}):
\begin{enumerate}
  ...
  \item The crossover $\eta^{\mathrm{DTA}} = \eta^{\mathrm{WS}}$ occurs at
        ...
        For $H$ large ($p_d \approx 0$), DTA's routing-overhead term is smaller
        than WS's stealing-overhead term at \emph{all} loads (Remark~\ref{rem:regime_compare_scope}
        --- this is an overhead-only statement, not a claim that DTA's full
        competitive ratio is uniformly below WS's; see Figure~\ref{fig:cr_vs_rho}).
...
\[
  \Gamma_{\mathrm{WS}}(\varrho_0) \;=\; \frac{N(1-\varrho_0)}{\varrho_0},
\]
which diverges as $\varrho_0 \to 0$.  At $N=4$ and $\varrho_0 = 0.1$
(10\% load): $\Gamma_{\mathrm{WS}} \approx 4 \times 0.9 / 0.1 = 36$
CAS operations per useful task (each additionally paying a cache-coherence
broadcast to the other $N-1=3$ cores, per Proposition~\ref{prop:cas_storm}).
...
+\varrho_0 p_d^2/(N-1)$, overload contagion) is negligible at $N=256$ and does not affect
...
\lfloor N/2\rfloor$ (a dimensionless integer, at most $N/2$). The
condition required, $\rho \leq H/(H+1)$, is on the \emph{system load}
$\rho$ relative to the admission threshold $H$, and is essentially
independent of $N$: since $H \geq 100$ in every configuration this
paper considers, the condition holds up to $\rho \leq 0.99$ regardless
of $N$, so Assumption~\ref{ass:threshold} is satisfied at every load
and worker count used elsewhere in this paper, including the $N=4$ CI
...
The quantity $\Delta_{\mathrm{mf}}$ in~\eqref{eq:dta_local_opt} is a gap
between two \emph{expected} costs (since $c(q)=q$ is linear, by Little's
law this is exactly a gap between mean queue lengths), so what is needed
here is convergence of the \emph{mean}, not the full distribution.
Proposition~\ref{prop:1overN} gives exactly this: the level-1-corrected
effective traffic intensity satisfies
$\rho^{(2)} = \rhostar_{\mathrm{MF}} + O(1/N)$. Since
$\rho \mapsto \rho/(1-\rho)$ is Lipschitz on any interval bounded away
from $1$ (with constant $(1-\rhostar_{\mathrm{MF}})^{-2}$, finite
whenever the system is not at the saturation boundary), the corrected
mean queue length satisfies
$\bar q^{(2)} = \bar q_{\mathrm{MF}} + O(1/N)$, and by
Assumption~\ref{ass:ergodic} (ergodicity) this $O(1/N)$ mean-field gap
is exactly the leading-order contribution to $\Delta_{\mathrm{mf}}$ as
$N\to\infty$. By Assumption~\ref{ass:bounded_tau}, information staleness introduces
...
Combining this with the Gumbel-CLT fluctuation term (Theorem~\ref{thm:fj_first},
makespan fluctuations scale as $O(W^{-1/2})$ relative to the mean), the two
error sources add:
\[
  \Delta_{\mathrm{mf}} \;=\; O\!\left(\frac{1}{N}\right) + O\!\left(\frac{1}{\sqrt{W}}\right)
  \quad\text{as } W \to \infty \text{ at fixed } N,
\]
i.e.\ $\Delta_{\mathrm{mf}}$ stays bounded (by an $N$-dependent constant) as
$W\to\infty$; it need not vanish on its own for Step~3 below to go through.

\medskip
\noindent\textbf{Step~3 ($\eta_{\mathrm{util}} \to 1$).}
The oracle makespan satisfies
$C_{\max}^*(\mathcal{I}^{\mathrm{DTA}}) = \Theta(W/(N\mu))$, which grows
without bound as $W \to \infty$ at fixed $N$. Since $\Delta_{\mathrm{mf}}$ is
bounded by Step~2, the ratio below vanishes regardless of whether
$\Delta_{\mathrm{mf}}$ itself does. From~\eqref{eq:dta_local_opt}:
\[
  \eta_{\mathrm{util}}(\mathrm{DTA})
  \;=\;
  \frac{\mathbb{E}[C_{\max}(\mathrm{DTA})]}{C_{\max}^*(\mathcal{I}^{\mathrm{DTA}})}
  ...
  \;=\;
  1 + O\!\left(\left(\frac{1}{N}+\frac{1}{\sqrt{W}}\right) \cdot \frac{N\mu}{W}\right)
  \;=\; 1 + O\!\left(\frac{1}{W}\right) + O\!\left(\frac{N}{W^{3/2}}\right),
\]
which tends to~$1$ as $W \to \infty$ at fixed $N$, establishing the theorem
for every $\varepsilon > 0$ by choosing $W_0 = W_0(N,\varepsilon)$.
\end{proof}
...
\noindent By Definition~\ref{def:J}, $\mathcal{J}(\pi;\gamma)$ is built from
$\mathbb{E}[C_{\max}(\pi)]$, the \emph{actual} expected makespan, not the
oracle $C_{\max}^*(\mathcal{I}_\pi)$; the two are related by
$\mathbb{E}[C_{\max}(\pi)] = \eta_{\mathrm{util}}(\pi)\cdot C_{\max}^*(\mathcal{I}_\pi)$
(the decomposition of Theorem~\ref{thm:decomp}). The exact crossover condition
therefore carries an $\eta_{\mathrm{util}}$ factor on each side: DTA dominates
WS ($\mathcal{J}_{\mathrm{DTA}} < \mathcal{J}_{\mathrm{WS}}$) if and only if
\begin{equation}\label{eq:gamma_star_exact}
  \gamma \;>\; \gamma^*
  \;\triangleq\;
  \frac{\eta_{\mathrm{util}}(\mathrm{DTA})\,C_{\max}^*(\mathcal{I}^{\mathrm{DTA}})
        - \eta_{\mathrm{util}}(\mathrm{WS})\,C_{\max}^*(\mathcal{I}^{\mathrm{WS}})}{
        (\beta_{\mathrm{WS}} - \beta_{\mathrm{DTA}})\cdot T}.
\end{equation}
By Theorem~\ref{thm:dta_info_opt} and Remark~\ref{rem:ws_opt},
$\eta_{\mathrm{util}}(\mathrm{DTA}), \eta_{\mathrm{util}}(\mathrm{WS})
\to 1$ as $W\to\infty$ (at fixed $N$), so in the large-workload limit the
$\eta_{\mathrm{util}}$ factors drop out and~\eqref{eq:gamma_star_exact}
reduces to the oracle-only expression used below:
\begin{equation}\label{eq:gamma_star}
  \gamma^*
  \;\xrightarrow{W\to\infty}\;
...
\end{equation}
where we used~\eqref{eq:info_gap_2socket} for the numerator. This limiting
form is what the numerical estimate below applies; at finite $W$,
\eqref{eq:gamma_star_exact} should be used directly with measured
$\eta_{\mathrm{util}}$ values if a higher-precision threshold is needed.
...
\medskip
\noindent\textbf{Errata from the verification process.}
The review described above surfaced and corrected several substantive
errors relative to earlier drafts, most notably a sign error in the
Level-1 pair-correlator analysis of \S\ref{sec:contagion-scope}'s
chapter (the true effect is overload \emph{contagion}, not
anti-correlation), a wrong-order asymptotic in the competitive-ratio
analysis of \S\ref{chap:competitive_ratio} (Gumbel $O(\ln N)$ in place
of an earlier Gaussian-tail $O(\sqrt{\ln N})$), and a symbol conflation
between the admission threshold $H$ and the hop-count bound $h_{\max}$
in Assumption~\ref{ass:threshold}. Each correction and its downstream
consequences are noted at the relevant point in the text; we record
them here together in the interest of transparency.

A subsequent, independent audit pass against the \dtact{} source
implementation surfaced nine further issues, since corrected: the
warehouse deflection trigger is $\lfloor N/2\rfloor$ failed hops, not
$N-1$, which changes the warehouse input rate
(Definition~\ref{def:warehouse-rates}) and every downstream stability and
crash-time result that depends on it; the CAS-storm overhead term
(Proposition~\ref{prop:cas_storm}) had a spurious extra factor of $(N-1)$; the
per-worker spectral-gap remark understated the mixing-time exponent by
a factor of $N$; the Foster--Lyapunov drift bound
(Theorem~\ref{thm:lyapunov}) originally substituted a mean-field
\emph{average} arrival rate where a pathwise, per-state bound was
required, and has been replaced with an unconditional pathwise proof
valid for every $\varrho_0$; Theorem~\ref{thm:bounded-wait}'s waiting-time
guarantee required an explicit net-drain assumption
(Assumption~\ref{ass:wh-net-drain}) that was previously left implicit;
the mean-field convergence step in
Theorem~\ref{thm:dta_info_opt} cited the wrong proposition and has been
re-derived via Proposition~\ref{prop:1overN} and Little's law; the
$\gamma^*$ crossover threshold (\eqref{eq:gamma_star_exact}) now carries
its $\eta_{\mathrm{util}}$ factors explicitly, with the earlier
oracle-only expression identified as its $W\to\infty$ limit; the Stage-W
overhead bound was tightened from an equality to an inequality to
account for integer ceiling effects; and the CAS retry-count bound
$\mathbb{E}[R_i] < \infty$ now has an explicit fairness-assumption proof
in place of an informal justification. As with the corrections above,
each fix and its consequences are recorded at the relevant point in the
text.
