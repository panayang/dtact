...
\begin{abstract}
We present a unified formal analysis of the \textsc{DTA} distributed task
scheduler ($N$ workers, SPSC mailboxes, Vyukov MPMC warehouse, hop-bounded
deflection).  Using a second-quantisation Fock-space algebra and kinetic
theory, we establish across seven parts:
...
\textbf{(VI)}~an information-theoretic cost framework proving DTA is
information-optimal ($\eta_{\mathrm{util}}\to 1$) and that its
information-acquisition cost is $\Omega(\log N)$ lower than work-stealing
on NUMA topologies, with a concrete crossover analysis for dual-socket systems.
All results are accompanied by Monte Carlo, ODE, and plotting scripts.
\end{abstract}
...
contention not captured by the DTA mean-field model.
...
does \dtact{} outperform \tokio{} by 2--6$\times$ in practice?}
...
information-theoretic framework that separates them.  We prove that DTA
is \emph{information-optimal} (it achieves the best possible makespan
given its information set), and we show that on a NUMA topology its
information-acquisition cost is $\Omega(\log N)$ times lower than WS.
On a representative dual-socket system this cost advantage explains
the observed throughput gap at moderate to large task counts.
...
\begin{enumerate}[label=(\textup{A\arabic*})]
  ...
  \item\label{ass:threshold}
        ...
        DTA sets $H = \lfloor N/2 \rfloor$; the assumption reduces to
        $H \geq H_g^*$, which for the M/M/1/$H$ case ($H^* =
        \lceil\rho/(1-\rho)\rceil$) is equivalent to
        $\rho \leq 1 - 2/N$ --- satisfied for $\rho < 1$ at any $N \geq 4$
        with mild margin (e.g.\ $\rho \leq 0.97$ at $N = 64$).
...
DTA's deflection threshold $H = \lfloor N/2\rfloor$ is the
\emph{maximum hop count}, determined at compile time.  The condition
$\rho \leq 1 - 2/N$ is a \emph{mild sub-saturation requirement}: it
excludes only the last $2/N$ fraction of load before saturation.
In particular it does NOT require $H \geq 1$ in any non-trivial sense;
the condition is on the \emph{system load} $\rho$, not on $H$
independently.  For typical server deployments ($\rho \leq 0.9$,
$N \geq 8$), Assumption~\ref{ass:threshold} is automatically satisfied.
\end{remark}
...
Hence \emph{both} DTA and WS achieve $\alpha \to 1$;
their performance difference lies entirely in $\eta_{\mathrm{info}}$
and the information acquisition rate $\beta$, analysed in
\S\ref{sec:info_cost}--\ref{sec:numa_concrete}.
\end{remark}
...
parameters for a modern dual-socket server (e.g.\ AMD EPYC 7763 or
Intel Xeon Platinum 8380, both of which match this profile):
\begin{center}
\begin{tabular}{lll}
\toprule
...
(Theorem~\ref{thm:dta_info_opt}) --- information-optimal within its
observation class;
(iii)~both DTA and WS satisfy $\eta_{\mathrm{util}} \to 1$, so their entire
performance difference lies in the information gap and acquisition cost;
...
\bigskip
\noindent\textbf{Overview.}
This part synthesises the contributions of the paper, characterises
the range of applicability of the developed methods and the DTA
algorithm, and identifies the most promising directions for future work.
The information-theoretic cost framework developed in Part~VI provides
the analytical basis for the applicability conclusions in \S\ref{sec:dta_scope}.
...
The spawn-efficiency benchmarks confirm 2.3--6.2$\times$ speedup over
Tokio at task counts $\geq 1$k.
...
\begin{enumerate}
  ...
  \item \textbf{Adversarial worst-case BoT bound without mean-field.}
        A competitive-ratio upper bound derived purely from work
        conservation and hop-count geometry, requiring no probabilistic
        assumption.
  \item \textbf{Information-theoretic scheduler cost framework.}
        ...
        gap and utilisation efficiency; a proof that DTA achieves
        $\eta_{\mathrm{util}} \to 1$ (information-optimal); and a
...
\begin{enumerate}
  ...
  \item Bounded waiting time theorem with explicit closed-form ceiling
        $W_{\max}(\lambda,\mu,N,H)$ and $k$-bounded fairness corollary.
...
\hyperref[chap:kinetic_theory]{Performance~II} closes the hierarchy at level~1.
The level-2 correction introduces triplet correlators
$g_3(q, q', q'')$ and is expected to yield $O(1/N^2)$ accuracy for
$N = 4$.  For the DTA benchmark this could reduce the mean-field
prediction error from $\approx 10\%$ to $\approx 1\%$.
...
\bigskip
\noindent\textbf{Summary.}
This paper has established that the DTA distributed task scheduler is
simultaneously \emph{provably correct} (four theorems), \emph{provably
live} (bounded wait, starvation freedom), \emph{quantitatively
characterised} (mean-field + BBGKY + stability ladder + makespan),
\emph{competitively near-optimal} (adversarial ratio $\to 1$ at high load),
and \emph{empirically validated} against public CI benchmarks.
The kinetic-theory and operator-algebraic methods introduced here
are general-purpose tools for distributed-systems analysis and are
expected to find application well beyond the specific scheduler studied.
The information-theoretic framework of Part~VI formalises this claim:
it provides a reusable methodology for comparing any pair of schedulers
on the basis of both makespan and information-acquisition cost.
