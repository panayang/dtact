...
(so the system is stable by Theorem~\ref{thm:L1}).
...
By Theorem~\ref{thm:L1}, DTA is stable under $\varrho_0 < 1$.
...
\subsection*{Future Work: Restricted DAG Scheduling}
\label{sec:dag_future}

Although DTA does not support general DAG workloads, a restricted result
is achievable for \emph{fork-join DAGs with bounded critical-path depth $d$}:

\begin{conjecture}[Fork-join DTA bound]\label{conj:dag}
Consider a fork-join DAG $G=(V,E)$ with $|V| = M$ independent leaf tasks
(all at depth $0$) and $d$ join levels.  If a two-phase protocol is used ---
(1) DTA schedules all leaf tasks; (2) join tasks are submitted only after all
predecessors complete (enforced by a counter at the application layer) --- then
\[
  \mathbb{E}[C_{\max}] \;\le\; \frac{T_1}{N} \;+\; d \cdot \delta_{\mathrm{sched}},
\]
where $\delta_{\mathrm{sched}}$ is the scheduling latency of one synchronisation
round (application-layer counter check plus one DTA submission).
\end{conjecture}

The key insight is that DTA's bounded-hop deflection provides load balancing
within each fork-join level, and the $d$-level join overhead is additive
rather than multiplicative.  A formal proof requires bounding the
\emph{synchronisation delay} between levels, which depends on
$\delta_{\mathrm{hop}}$ and the spectral gap $\gamma$ (L2 stability).
This is left as future work; the present paper establishes Conjecture~\ref{conj:dag}
empirically for $d \le 3$ in the benchmark configuration.
