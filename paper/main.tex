...
livelock freedom, linearizability);
...
\textbf{(V)}~empirical validation against public Criterion CI benchmarks,
...
We adopt a notation inspired by \emph{second quantization}~\cite{dirac1927,jordan_wigner1928,fock1932,landau_qm,feynman_iii,derrida2007,schutz2001} from
...
\begin{enumerate}[label=(\textup{A\arabic*})]
  \item\label{ass:poisson} Tasks arrive as a Poisson process with rate $\lambda$;
        service times are i.i.d.\ $\mathrm{Exp}(\mu)$; load
        $\rho = \lambda/(N\mu) < 1$.
  \item\label{ass:threshold} The deflection threshold satisfies
        $H \geq \lceil\rho/(1-\rho)\rceil + 1$.
  \item\label{ass:bounded_tau} Information staleness is bounded:
        $\tau_{gh} \leq \bar\tau < \infty$ for all $g,h \in G$.
\end{enumerate}
\end{assumption}
...
\begin{proof}
We model the routing decision at each arrival epoch as a
\emph{partially observed Markov decision process} (POMDP) with observable
state $q_g \in \{0,\ldots,H\!+\!1\}$ (worker $g$'s own queue length)
and action space $\{\mathrm{accept}, \mathrm{deflect}\}$.

The instantaneous cost $c(q) = q$ (sojourn contribution, by Little's
law~\cite{little1961}) is convex and non-decreasing in $q$.
For the M/M/$1$/$H$ queue with overflow routing, the optimal policy
minimising $\sum_t c(q(t))$ subject to observing only $q(t)$ is a
\emph{threshold policy}: accept if $q \leq H^*$, deflect otherwise,
where $H^* = \lceil\rho/(1-\rho)\rceil$ (see~\cite{stidham1985,weber1990}).
By~\ref{ass:threshold}, $H \geq H^*$, so DTA's policy is optimal within
$\mathcal{I}^{\mathrm{DTA}}$, i.e.\ $\mathbb{E}[C_{\max}(\mathrm{DTA})]
= C_{\max}^*(\mathcal{I}^{\mathrm{DTA}}) + \Delta$, where $\Delta$
is the mean-field approximation error.

By Theorem~\ref{prop:mf-recovery} (mean-field recovery), the steady-state
distribution converges to $q^*$ with error $O(1/N)$ in total variation.
Under the Gumbel CLT (Theorem~\ref{thm:fj_first}), $\Delta = O(W^{-1/2})$
and $C_{\max}^*(\mathcal{I}^{\mathrm{DTA}}) = \Theta(W/(N\mu))$.
Hence $\Delta / C_{\max}^*(\mathcal{I}^{\mathrm{DTA}}) = O(W^{-1/2}) \to 0$,
completing the proof.
\end{proof}

\begin{remark}
The same argument applies to WS: WS's random-victim steal policy is
optimal within $\mathcal{I}^{\mathrm{WS}}$ (optimal random search
under uniform ignorance).  Hence \emph{both} schedulers achieve
$\eta_{\mathrm{util}} \to 1$; the gap between them is entirely in
$\eta_{\mathrm{info}}$, analysed concretely in \S\ref{sec:numa_concrete}.
\end{remark}
...
If $n$ workers simultaneously attempt a compare-and-swap on a single
cache line using randomised exponential backoff, the expected
\emph{per-worker} effective latency satisfies
\begin{equation}\label{eq:cas_lb}
  c_{\mathrm{CAS}}(n) \;=\; \Omega(\log n)\cdot c_{\mathrm{CAS}}^{(0)},
\end{equation}
where $c_{\mathrm{CAS}}^{(0)}$ is the uncontended CAS latency.
\end{lemma}

\begin{proof}
With $n$ concurrent contenders and uniform random backoff, each backoff
round succeeds with probability $1/n$ for any given worker.
By the coupon-collector argument~\cite{mitzenmacher2017},
the expected number of rounds until all $n$ workers succeed is
$\sum_{k=1}^{n} n/k = n H_n = \Theta(n\log n)$ total rounds,
giving $\Theta(\log n)$ rounds per worker.  Each round costs
$c_{\mathrm{CAS}}^{(0)}$ (one CAS attempt), establishing~\eqref{eq:cas_lb}.
\end{proof}
...
and any system analysed by the ``power of two choices'' framework.
...
\url{https://github.com/bheisler/criterion.rs}, 2025.
...
``Complexity of machine scheduling under precedence constraints,''
...
\bibitem{tokio}
\textsc{Tokio Contributors},
\textit{Tokio --- An Asynchronous Rust Runtime},
\url{https://github.com/tokio-rs/tokio}, 2026.
Version 1.x; \texttt{multi\_thread} scheduler with work-stealing deque.

\bibitem{landau_qm}
L.~D.~Landau and E.~M.~Lifshitz,
\textit{Quantum Mechanics (Non-Relativistic Theory)},
3rd~ed., Course of Theoretical Physics, Vol.~3.
Butterworth-Heinemann, Oxford, 1977.
\S\S~64--65 (occupation numbers and second quantisation).
...
Chapters~1--2 (state-vector formalism) and Chapter~4 (identical particles).

\bibitem{feynman_ii}
R.~P.~Feynman, R.~B.~Leighton, and M.~Sands,
\textit{The Feynman Lectures on Physics, Vol.~II: Mainly Electromagnetism and Matter}.
Addison-Wesley, Reading, MA, 1964.
Chapter~1 (field as a collection of harmonic oscillators).

\bibitem{dirac1927}
P.~A.~M.~Dirac,
``The quantum theory of the emission and absorption of radiation,''
\textit{Proc.\ R.\ Soc.\ Lond.\ A}, 114(767):243--265, 1927.
[First introduction of creation and annihilation operators.]

\bibitem{jordan_wigner1928}
P.~Jordan and E.~Wigner,
``\"Uber das Paulische \"Aquivalenzverbot,''
\textit{Z.\ Phys.}, 47(9--10):631--651, 1928.
[Fermionic occupation-number representation; Jordan--Wigner transformation.]

\bibitem{fock1932}
V.~Fock,
``Konfigurationsraum und zweite Quantelung,''
\textit{Z.\ Phys.}, 75(9--10):622--647, 1932.
[Introduction of Fock space and the occupation-number formalism.]


\bibitem{shannon1948}
C.~E.~Shannon,
``A Mathematical Theory of Communication,''
\textit{Bell Syst.\ Tech.\ J.}, vol.~27, pp.~379--423 and 623--656, 1948.

\bibitem{cover_thomas2006}
...
\bibitem{loom2023}
Tokio Contributors,
``Loom: Permutation Testing for Concurrent Rust Code,''
\url{https://github.com/tokio-rs/loom}, 2024.
[Concurrency model checker used to validate lock-free data structures.]
...
\emph{IEEE Transactions on Automatic Control}, vol.~30, no.~8,
pp.~705--713, 1985.

\bibitem{weber1990}
R.~R.\ Weber and G.~Weiss,
``On an index policy for restless bandits,''
\emph{Journal of Applied Probability}, vol.~27, no.~3,
pp.~637--648, 1990.

\bibitem{mitzenmacher2017}
M.~Mitzenmacher and E.~Upfal,
\emph{Probability and Computing: Randomization and Probabilistic
